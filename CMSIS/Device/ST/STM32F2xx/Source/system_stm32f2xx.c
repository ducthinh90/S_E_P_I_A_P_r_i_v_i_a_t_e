...
extern uint16_t PLL_M;
extern uint16_t PLL_N;
extern uint16_t PLL_P;
extern uint16_t PLL_Q;
...
#define VECT_TAB_OFFSET  0 //0x8000 /*!< Vector Table base offset field. 
...
/******************************************************************************/
  ...
  {
    /* HCLK = SYSCLK / 1*/
    RCC->CFGR |= RCC_CFGR_HPRE_DIV1;// 120 mhz max
      
    /* PCLK2 = HCLK / 2*/
    RCC->CFGR |= RCC_CFGR_PPRE2_DIV2;// 60 mhz max

    
    /* PCLK1 = HCLK / 4*/
    RCC->CFGR |= RCC_CFGR_PPRE1_DIV4;// 30 mhz max
